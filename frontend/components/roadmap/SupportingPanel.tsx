    ...
    <div className="space-y-6">
      ...
      {(activeStage.recommended_projects?.length ?? 0) > 0 && (
        <motion.div
          ...
          className="p-5 border border-zinc-100 bg-white"
        >
          ...
          <ul className="space-y-2">
            {activeStage.recommended_projects!.map((proj, i) => (
              <li
                key={i}
                className="flex items-center gap-2 text-sm text-zinc-700 font-medium"
              >
                <span className="text-[9px] font-bold text-zinc-400 tracking-widest w-4 shrink-0">
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {related.map((skill, i) => (
              <span
                key={i}
                className="text-[10px] font-bold tracking-widest uppercase px-2.5 py-1 bg-zinc-50 border border-zinc-200 text-zinc-500"
              >
                {skill}
              </span>
            ))}
      ...
      {nextStage && (
        <motion.div
          ...
          className="p-5 border border-zinc-900 bg-zinc-900 text-white"
        >
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            {nextStage.skills?.slice(0, 3).map((skill, i) => (
              <span
                key={i}
                className="text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 bg-white/10 text-zinc-300"
              >
                {skill}
              </span>
            ))}
            {(nextStage.skills?.length ?? 0) > 3 && (
              <span className="text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 bg-white/10 text-zinc-400">
